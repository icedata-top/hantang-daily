...
public class TodayStaticDataJob {
    ...
    private static final StaticDataFilter STATIC_DATA_FILTER;
    ...
    static {
        try {
            ...
            STATIC_DATA_FILTER = new StaticDataFilter(properties);
        } catch (IOException e) {
            e.fillInStackTrace();
        ...
        for (String keyword : KEYWORDS) {
            ...
            for (int page = 1; page <= pageTotal && isPubdateInLimit; page++) {
                ...
                for (VideoStaticDO videoStaticDO : curVideoStaticDOList) {
                    ...
                    if (!aidSet.contains(aid) && STATIC_DATA_FILTER.shouldRetain(videoStaticDO)) {
                        aidSet.add(aid);
